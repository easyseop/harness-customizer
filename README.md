...
| | 현재 | Phase 2 (실제 호출) | 더 현실적인 접근 |
|-|------|------------------|----------------|
| 판정 방식 | LLM 예측 | 실제 에이전트 1~2회 호출 | 지시문 명확성 체크 |
| 한계 | LLM이 LLM을 흉내 낸 것. 수정 효과를 알 수 없음 | **1~2회 호출로는 통계적으로 의미 없음.** 같은 md라도 매 실행마다 응답이 다름. 테스트 시나리오가 뻔해서 당연히 원하는 답이 나옴 | — |
| 개선 효과 | — | "없는 것보다는 낫다" 수준. 획기적 개선 아님 | 아래 참고 |

**agent .md 검증의 근본 한계**: LLM은 비결정론적입니다. 지시문을 아무리 잘 써도 "항상 따른다"는 보장이 없고, 이를 테스트로 완전히 검증하는 것은 현재 업계에서도 미해결 문제입니다.

**현실적인 대안 — 지시문 명확성 체크**:

행동 신뢰도는 지시문이 얼마나 구체적인지에 비례합니다. 자동 테스트보다 지시문 품질을 높이는 게 더 효과적입니다.
...
✅ 구체적인 지시 (신뢰도 높음)
"외부 API 연동이 언급되면 반드시 아래 질문을 한다:
 '사설 CA 인증서를 신뢰해야 하는 환경인가요?'"
```

harness-judge가 현재 이 체크를 수행합니다 (트리거 조건 명시 여부, 행동 구체성, 기존 규칙 충돌 여부).

**장기 과제 — Eval 프레임워크**:

완전한 행동 검증은 [promptfoo](https://www.promptfoo.dev/), LangSmith 같은 eval 도구로 다중 실행(10회 이상) + 채점 기준 정의가 필요합니다. 현재 이 봇의 범위 밖입니다.
