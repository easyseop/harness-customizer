...
각 파일의 역할과 "예측으로 판정"이 왜 한계인지를 함께 설명합니다.

---

**`gate .sh` — 커밋 전 자동 차단 스크립트**

> 코드에 금지된 패턴(외부 서비스 import, 하드코딩된 키 등)이 있으면 커밋 자체를 막습니다.

| | 현재 | Phase 1 | Phase 2 |
|-|------|---------|---------|
| 판정 방식 | 예측으로 판정 | ✅ 실제 실행으로 판정 | ✅ |
| 한계 / 개선 | 시뮬레이터가 "아마 탐지될 거예요"라고 예측하면 PASS 처리. 스크립트 문법 오류나 탐지 누락이 있어도 발견 못 함 | 테스트 코드 파일을 직접 생성해 실제 bash로 실행. 탐지 결과가 실제 ❌ GATE FAIL로 나오는지 확인 | — |

---

**`agent .md` — AI 에이전트의 역할·질문·판단 기준 정의 파일**

> 인터뷰어가 어떤 질문을 할지, 평가자가 어떤 기준으로 판정할지 등 AI 행동을 정의합니다.

| | 현재 | Phase 1 | Phase 2 |
|-|------|---------|---------|
| 판정 방식 | 예측으로 판정 | 예측으로 판정 | ✅ 실제 호출로 판정 |
| 한계 / 개선 | 시뮬레이터가 "이 에이전트는 이렇게 물어볼 것 같다"고 예측. 실제로 에이전트를 실행한 게 아니라 LLM이 LLM을 흉내 낸 것. 수정 후 행동이 정말 바뀌었는지 알 수 없음 | 개선 불가 (bash로 md 파일 실행 불가) | 수정된 내용을 컨텍스트로 주입해 에이전트를 실제 호출. 테스트 시나리오 대화에서 원하는 질문·판단이 나오는지 직접 확인 |

---

**`CLAUDE.md` / `yaml` — AI 전역 행동 지침 및 선언적 규칙**

> CLAUDE.md는 모든 작업에 적용되는 절대 규칙 모음. yaml은 허용·금지 목록을 선언하는 설정 파일입니다.

| | 현재 | Phase 1 | Phase 2 |
|-|------|---------|---------|
| 판정 방식 | 예측으로 판정 | 구조 검증 추가 | 구조 검증 추가 |
| 한계 / 개선 | 제안한 규칙이 실제로 기존 규칙과 충돌하는지, 섹션이 올바른 위치에 들어갔는지 확인하지 않음 | 파일에 실제로 적용됐는지, 섹션 구조가 올바른지 텍스트 수준에서 검증. 단, 이 규칙이 AI 행동에 실제로 반영되는지까지는 확인 불가 (런타임 영향이라 실행 전 검증 한계) | — |
