...
해볼 수는 있지만 크게 의미가 없습니다. AI는 같은 지시문을 줘도 매번 응답이 조금씩 다릅니다. 1~2번 실행해서 원하는 질문이 나왔다고 해서, 실제 사용에서도 항상 나온다는 보장이 없습니다. 10번 중 1번만 나와도 테스트에서는 통과할 수 있습니다.

**그러면 어떻게 해야 하나?**

테스트보다 지시문 자체를 잘 쓰는 게 더 효과적입니다. AI는 지시가 구체적일수록 일관되게 따릅니다.

```
❌ 이렇게 쓰면 안 됨
"관련 질문을 하도록 한다"
→ "관련"이 뭔지 AI가 매번 다르게 해석

✅ 이렇게 써야 함
"외부 API 연동이 언급되면 반드시 이 질문을 한다:
 '사설 CA 인증서를 신뢰해야 하는 환경인가요?'"
→ 언제, 정확히 어떤 말을 할지 명시
```

그래서 harness-judge가 수정 후 지시문이 충분히 구체적으로 쓰였는지를 체크합니다. 동작을 보장하진 못하지만, 지시문 품질을 높이는 것이 현재 할 수 있는 가장 현실적인 개선입니다.

**장기 과제**

완전한 검증은 promptfoo, LangSmith 같은 eval 도구로 동일 시나리오를 10회 이상 실행해 몇 번이나 원하는 행동이 나오는지 측정하는 방식이 필요합니다. 이는 이 봇의 현재 범위 밖입니다.
