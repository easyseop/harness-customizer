...
```
Step 0 — 회귀 테스트
  수정 전 md에서 기존 규칙 추출 → 수정 후에도 동일하게 동작하는지 확인
  기존 동작이 하나라도 깨지면 즉시 Fail, 이후 단계 진행 안 함

Step A-1 — 규칙 충돌 감지
  신규 규칙 × 기존 규칙 전체를 의미 수준에서 대조
  "이미 답변한 것은 묻지 않는다" × "CA 질문을 항상 한다" → 충돌 경고 발생

Step A-2 — 커버리지 체크
  추가된 규칙마다 테스트 케이스가 있는지 확인
  빠진 규칙은 자동으로 시나리오 생성 후 추가

Step B — Cousin Prompt + 경계 조건
  Cousin: "결제 API" → "PG사", "카드 모듈", "외부 결제 서비스" 표현 변형 강건성
  경계: 트리거 조건 딱 걸치는 애매한 케이스 (예: "webhook 수신", "내부망 외부 포트")

Step C — 페어와이즈 비교 10회
  Before/After 같은 시나리오로 10회 비교
  judge가 어느 쪽이 목표 행동에 더 가까운지 선택 → 8/10 이상 After 시 PASS

Step D — 멀티턴 시나리오 (조건부)
  규칙에 "이미", "반복", "다시", "중복" 키워드 있을 때만 실행
  3~5턴 대화 시뮬레이션으로 상태 의존적 동작 검증
```
