...
**agent `.md` 수정 시 (5단계 검증)**
1. Step 0  — 회귀 테스트: 기존 동작이 수정 후에도 유지되는지
2. Step A-1 — 규칙 충돌 감지: 신규 규칙과 기존 규칙 간 의미적 충돌 탐지
3. Step A-2 — 커버리지 체크: 추가된 규칙이 테스트에 모두 커버되는지
4. Step B  — Cousin Prompt + 경계 조건: 표현 변형 강건성 + 트리거 경계 케이스
5. Step C  — 페어와이즈 10회: Before vs After 비교, 8/10 이상 After 선택 시 PASS
6. Step D  — 멀티턴 시나리오: "이미/반복/다시" 등 키워드 포함 규칙일 때만 실행
